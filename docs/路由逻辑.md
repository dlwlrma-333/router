...
- 关键代码目录：
  ...
  - `internal/admin/repository/groupmodelroute`
  - `internal/relay`
...
4. 实际可选渠道来源不是 `channel_models`，而是运行时展开表 `group_model_routes`。
5. `group_models` 负责表达“分组对外提供哪些模型”；`channel_models` 负责表达“渠道支持哪些模型、模型使用哪个上游端点”；`group_model_routes` 负责表达“某个分组下，这个模型当前能走哪些渠道”。
...
- `group_model_routes`：运行时能力展开表，描述某个分组下某模型可用的渠道集合。
...
- `group_model_routes` 是运行时展开表，不是主配置表。
- 这里的“运行时”专指真实请求正在经过 relay 链路时，Router 为了快速选路而直接读取的数据。
...
3. `group_model_routes` 决定这个分组下、这个模型当前有哪些可用渠道。
...
- `group_model_routes` 决定“Router 运行时允许这个分组实际用什么渠道承载这些模型”。
...
必须进入 `group_model_routes` 才算候选池成员。
...
3. 当前分组下，该模型在 `group_model_routes` 里到底有几个 `enabled=true` 候选。
