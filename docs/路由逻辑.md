...
6. 请求内切换是否发生，受 `RelayRetryLimit`、错误状态码、是否指定渠道、候选池规模共同影响。
7. 当前请求内切换上限由 `system_settings.RelayRetryLimit` 控制，`0` 表示关闭。
...
flowchart TD
    ...
    B -- 否 --> X[直接返回错误]
    B -- 是 --> C[读取 RelayRetryLimit]
    C --> D{RelayRetryLimit > 0}
    D -- 否 --> X
    D -- 是 --> E[进入重试循环]
...
1. 请求内切换上限取自运行时配置 `RelayRetryLimit`。
...
- `system_settings.RelayRetryLimit=0`
...
还要满足 `RelayRetryLimit > 0`、错误可重试、未指定渠道。
...
1. `system_settings.RelayRetryLimit` 是否存在，值是多少。
